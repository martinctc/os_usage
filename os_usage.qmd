...
# Read RDS back in and bind, taking the max value per date when sources overlap
# (different snapshots may have partial values for the same day).
full_dl <-
  ...
  group_by(date) %>%
  summarise(
    CRAN = suppressWarnings(max(CRAN, na.rm = TRUE)),
    PyPI = suppressWarnings(max(PyPI, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  mutate(
    CRAN = ifelse(is.finite(CRAN), CRAN, NA_real_),
    PyPI = ifelse(is.finite(PyPI), PyPI, NA_real_)
  ) %>%
  arrange(date)
```
