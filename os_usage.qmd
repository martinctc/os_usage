...
# Function to get GitHub traffic data
get_gh_data <- function(owner = "microsoft", repo, diff) {
  ...
  # Fetch clones data
  gh_json_clones <- gh(paste0(endpoint, "clones"), username = owner, repo = repo, .token = gh_pat)
  
  # Fetch views data
  gh_json_views <- gh(paste0(endpoint, "views"), username = owner, repo = repo, .token = gh_pat)
  ...
  # Combine clones and views data
  gh_out <- gh_json_clones %>%
    full_join(gh_json_views, by = "date")
    
  gh_out
}

gh_r_df <- get_gh_data(owner = "microsoft", repo = "vivainsights", diff = "R")
gh_py_df <- get_gh_data(owner = "microsoft", repo = "vivainsights-py", diff = "Py")
gh_all <-
  gh_r_df %>%
